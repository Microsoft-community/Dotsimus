...
const initialize = new Promise((resolve, reject) => {
  mongoose.connect('mongodb://127.0.0.1:27017/test', { useUnifiedTopology: true, useNewUrlParser: true, useFindAndModify: false }, function (error, response) {
    if (error) {
...
module.exports = {
  ...
  getWatchedKeywords: async function (userId, serverId) {
    let result;
    if (userId && serverId) {
      result = await WatchKeyword.find({ userId, serverId }).lean()
    } else {
      result = await WatchKeyword.find({}).lean();
    }
    return result;
  },

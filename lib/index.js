...
if (process.env.DEVELOPMENT !== 'true') Sentry.init({ dsn: 'https://cc4bb0af29884a99bd204804ea5c5ac6@o436175.ingest.sentry.io/5396750' });
    ...
    entireCollection.filter(watchedKeywordsCollection => watchedKeywordsCollection.serverId === serverId).map(watchedKeywordsGuild => {
      ...
      words.forEach(word => {
        if (message.content.toLowerCase().indexOf(word) != -1) client.users.fetch(watchedKeywordsGuild.userId, false).then((user) => {
          try {
            if (watchedKeywordsGuild.userId === message.author.id || isWatcherActive) return;
            const trackingNotice = (message.channel.permissionsFor(watchedKeywordsGuild.userId).serialize()['KICK_MEMBERS'] || message.channel.permissionsFor(watchedKeywordsGuild.userId).serialize()['BAN_MEMBERS']) ?
              ...
              new Discord.MessageEmbed()
                ...
                .setFooter(`Stop tracking with !unwatch command in ${serverName} server.`)
                .setColor('#7289da');
            if (message.channel.permissionsFor(watchedKeywordsGuild.userId).serialize()['VIEW_CHANNEL']) user.send(trackingNotice).catch(error => {
              console.info(

...
client.on('message', message => {
  ...
  getToxicity(message.content, message, false).then(toxicity => {
    ...
    if ((messageToxicity >= .80 || toxicity.insult >= .70) && (message.guild.members.cache.get(message.author.id).roles.cache.map(r => `${r}`)).length === 1 && serverId === '150662382874525696') {
      console.info(`${getTime()} #${message.channel.name} ${message.author.username}: ${message.content} | ${chalk.red((Number(messageToxicity) * 100).toFixed(2))}`)
      try {
    ...
    if (messageToxicity >= .81 || toxicity.insult >= .75) {
      console.info(`${getTime()} #${message.channel.name} ${message.author.username}: ${message.content} | ${chalk.red((Number(messageToxicity) * 100).toFixed(2))}`)
      ...
      if (messageToxicity >= .90 || toxicity.insult >= .90) {
        try {
          message.delete({ reason: "toxic message" });
          const role = message.guild.roles.cache.find(role => role.name === 'Muted'),
            member = message.guild.members.cache.get(message.author.id);

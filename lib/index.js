...
; (async function () {
  const filterEmoji = (message) => message.replace(/([\u2700-\u27BF]|[\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2011-\u26FF]|\uD83E[\uDD10-\uDDFF])/g, '')
  ...
  bot.on('message', function (user, userId, channelId, message, evt) {
    ...
    getToxicity(message).then(toxicity => {
      ...
      logger.info(`${getTime()} #${channelName} ${user}: ${message} | ${(Number(toxicity) * 100).toFixed(2)}`)
      if (toxicity > .81) {
        logger.info(new toxicityReport(+new Date, userId, user, message, toxicity))
        try {
          db.saveWarningDb(+new Date, userId, user, message, toxicity)
        } catch (error) {
          logger.info(error)
        }
        bot.sendMessage({
  ...
  async function getToxicity (message) {
    ...
    const sanitizedMessage = filterEmoji(message);

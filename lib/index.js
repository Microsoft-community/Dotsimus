          ...
          if(args[0] != undefined) {
            if(args[0].length == 1) {
             db.updateServerPrefix(
               ...
               serversConfigStore = []
               if (data) { message.channel.send("Prefix changed."); } if (data) client.guilds.cache.forEach(server => {
                db.saveServerConfig(
             ...
             message.channel.send("Max length is 1."); 
           }
        } else {
        	message.channel.send("Min length is 1."); 
        }
       }
        break;
      // document

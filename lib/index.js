      ...
      words.forEach(word => {
        if (message.content.toLowerCase().indexOf(word) != -1) client.users.fetch(watchedKeywordsGuild.userId, false).then((user) => {
          const trackingNotice = new Discord.MessageEmbed()
            ...
            .setColor('#7289da');
          if (message.channel.permissionsFor(watchedKeywordsGuild.userId).serialize()['VIEW_CHANNEL']) user.send(trackingNotice);
        });
      });
    })
  })
  ...
  if (message.content.startsWith(prefix)) {
    ...
    switch (command) {
      ...
      case 'track':
        ...
        try {
          db.watchKeyword(message.author.id, serverId, trackingWord).then({
      ...
      case 'unwatch':
      case 'untrack':
        db.removeWatchedKeyword(message.author.id, serverId).then({
          refreshWatchedCollection()
        })
        message.react("✅")
        break;

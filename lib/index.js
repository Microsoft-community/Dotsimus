const Discord = require('discord.io'),
  ...
  db = require('./db'),
  { discordToken, perspectiveKey } = require('../auth.json'),
...
; (async function () {
  ...
  bot.on('message', function (user, userId, channelId, message, evt) {
    ...
    getToxicity(message).then(toxicity => {
      ...
      logger.info(`${user}: ${message} | ${toxicity}`)
      if (toxicity > .80) {
        logger.info(new toxicityReport(+new Date, userId, user, message, toxicity))
        bot.sendMessage({
          to: channelId,
          message: `<@${userId}>, watch your language.`
        })
      }

      //  Disabling this in favor of custom alerts. Consider adding a default alert for new servers
      const adminRole = 71270107371802624
      if (toxicity > .70) {
        bot.sendMessage({
          to: '340976155123515396',
          message: `<@${adminRole}>, please investigate ${user}'s toxic message(${(Number(toxicity) * 100).toFixed(2)}) in ${channelName}: ${message}.`
        })
      }

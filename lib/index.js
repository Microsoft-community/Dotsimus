...
client.on('message', message => {
  ...
  getToxicity(message.content, message, false).then(toxicity => {
    ...
    const alerts = () => {
      ...
      db.getAlerts(message.channel.guild.id).then(alerts => alerts.forEach(a => {
        const investigationEmbed = new Discord.MessageEmbed()
          ...
          .addFields(
            {
              name: `🔎 Investigate user's message(${(Number(messageToxicity) * 100).toFixed(2)}, ${(Number(toxicity.insult) * 100).toFixed(2)})`, value: message.content.slice(0, 1024)
            },
    ...
    if ((((messageToxicity >= .70 || toxicity.combined >= .70) && isUserNew) || (messageToxicity >= .80 || toxicity.combined >= .80)) && serverId === '150662382874525696') {
      ...
      getLatestUserMessages(message.author.id).then(function (result) {
        ...
        const newInvestigationEmbed = new Discord.MessageEmbed()
          ...
          .addFields(
            {
              name: `🔎 Investigate **new user's** message(${(Number(messageToxicity) * 100).toFixed(2)}, ${(Number(toxicity.insult) * 100).toFixed(2)})`, value: message.content.slice(0, 1024)
            },
            { name: 'User', value: `<@${message.author.id}>`, inline: true },
            { name: 'User ID', value: message.author.id, inline: true },
            { name: 'Is user new?', value: isUserNew ? "Yes" : "No", inline: true },
        ...
        if (result.length === 2) {
          ...
          try {
            ...
            try {
              isUserNew ? db.saveWarningDb(+new Date, message.author.id, message.author.username, message.content, messageToxicity) : ''
              isUserNew ? client.channels.cache.get('745276933767430255').send('@here', newInvestigationEmbed) : alerts();
            } catch (error) {
              console.error(error)
            }

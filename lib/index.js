...
client.on('message', message => {
  ...
  watchedKeywordsCollection.then(entireCollection => {
    entireCollection.filter(x => x.serverId === serverId).map(x => {
      let words = x.watchedWords;
      words.forEach(word => {
        if (message.content.toLowerCase().indexOf(word) != -1) client.users.fetch(x.userId, false).then((user) => {
          user.send(`Tracked keyword: '${word}' mentioned here <https://discordapp.com/channels/${serverId}/${message.channel.id}/${message.id}>.`);
        });
      });
    })
  })
